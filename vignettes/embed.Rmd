...
The package supports embedding from three services: YouTube, Vimeo, and now Microsoft's Channel 9 (including the useR! 2016 presentations, thanks Microsoft!). 
...
Consider the URL for Don Knuth's talk:

`https://channel9.msdn.com/Events/useR-international-R-User-conference/useR2016/`**`Literate-Programming`**

The trick for these useR videos is that the `id` is just the last part of the URL.
...
The `embed_*()` family of functions is designed to hew to the API of each service, and to allow you to work more easily with each.
...
- Channel 9 - click the embed button to see options
...
Note: find Dean Attali's lighting talk
...
For Vimeo, there is a somewhat undocumented way (thanks @karthik) to do this. Word of warning, there seems to be no way to avoid the autoplay, so beware - also see some notes at Vimeo: (https://vimeo.com/forums/topic:49396).

```{r eval=FALSE}
div(align = "center", embed_vimeo(id = "98892825", fragment = "t=60"))
```
